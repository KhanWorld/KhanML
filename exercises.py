...
def exercise_template(language):
    # [Multilingual]
    path = os.path.join(os.path.dirname(__file__), "khan-exercises/exercises/%s/khan-exercise.html" % language)
    ...
    return contents
# [Multilingual]
@layer_cache.cache_with_key_fxn(lambda exercise_name, language: "exercise_contents_%s_%s" % (language, exercise_name), layer=layer_cache.Layers.InAppMemory)
def exercise_contents(exercise_name, language):
    # [Multilingual]
    contents = raw_exercise_contents("%s/%s.html" % (language, exercise_name))
